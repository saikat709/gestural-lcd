...
class HandDetector:
    ...
    def get_gesture_name(self):
        ...
        if self.is_thumbs_up():
            return "Thumbs Up"
        elif self.get_finger_count() == 2 and self.finger_status[0] == 1 and self.finger_status[1] == 1 and self.finger_status[4] == 1:
            return "Swag Mood"
        elif self.drawing_mood():
            return "Pointing something"
        elif self.tool_selection_mood():
            return "Selfie mood"
        elif self.hand_spread():
            return "Hand Spread"
    ...
    def find_index_tip_position(self, draw = True):
        ...
        return int(index_top.x * self.width), int(index_top.y * self.height)
